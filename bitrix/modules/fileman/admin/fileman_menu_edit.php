...
$multiSite = false;
$docRoot = $dbSitesList->Fetch()['DOC_ROOT'];
	...
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && is_array($ids) && check_bitrix_sessid())
	{
		...
		for($i = 0, $l = count($ids); $i < $l; $i++)
		{
			...
			else
			{
				...
				for($j = 0, $l = count($arAddLinksTmp); $j < $l; $j++)
				{
...
?>
	...
	<?else:?>
		...
		<td valign="top" width="50%">
			...
			<tr>
				<td valign="top" align="right"><?=GetMessage("FILEMAN_MENU_EDIT_ADDITIONAL_LINK")?></td>
				<td valign="top"><textarea rows="3" cols="30" name="additional_link_<?= $i?>" WRAP="off"><?for($j=0, $l=count($aMenuLinksItem[2]); $j<$l; $j++)echo htmlspecialcharsex($aMenuLinksItem[2][$j])."\n"?></textarea></td>
			</tr>
			<tr>
				<td valign="top" align="right"><?=GetMessage("FILEMAN_MENU_EDIT_CONDITION_TYPE")?></td>
				<td valign="top">
