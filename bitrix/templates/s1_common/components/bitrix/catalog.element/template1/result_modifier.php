...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		$arOffer['SUBSCRIPTION'] = (($arOffer['CATALOG_SUBSCRIBE'] == 'Y') ? true : false);

		$arResult['OFFERS'][$keyOffer]['SUBSCRIPTION'] = $arOffer['SUBSCRIPTION'];
		...
		$arOneRow = array(
			...
			'SUBSCRIPTION' => $arOffer['SUBSCRIPTION'],
...
$LINKED_ELEMENTS = $arResult["LINKED_ELEMENTS"];
if (sizeof($LINKED_ELEMENTS) > 0)
{
	$LINKED_ELEMENTS_ID = array();

	foreach($LINKED_ELEMENTS as $LINKED_ELEMENTS_VAL)
	{
		$LINKED_ELEMENTS_ID[] = $LINKED_ELEMENTS_VAL["ID"];
	}
	
	if ($element_result = CIBlockElement::GetList(array("SORT"=>"ASC","ID"=>"ASC"),array("ID"=>$LINKED_ELEMENTS_ID)))
	{
		if ($element_result->SelectedRowsCount() > 0)
		{
			unset($arResult["LINKED_ELEMENTS"]);

			while ($arElement = $element_result->GetNext())
			{
