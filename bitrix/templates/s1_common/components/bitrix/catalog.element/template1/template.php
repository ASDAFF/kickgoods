...
$arItemIDs = array(
	...
	'SUBSCRIPTION' => $strMainID.'_subscription',
...
if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	$canSubscription = $arResult['OFFERS'][$arResult['OFFERS_SELECTED']]['SUBSCRIPTION'];
}
else
{
	$canBuy = $arResult['CAN_BUY'];
	$canSubscription = $arResult['SUBSCRIPTION'];
}

if ($arResult["PROPERTY_66"]==39) {
	$pr = 1;
	?>
		<script>
			$(document).ready(function(){
				$('#show_ff').click(function(){
					$('.ff').css('display', 'block');
					$(this).css('opacity', '0');
					$('.ff input[name=form_text_4]').focus();
					$('.ff br').remove();
					$('.preorder-form-descr').text('Ожидаемая дата поставки - <?=$arResult["PROPERTY_67"];?>');
				});

				$('.ff').delegate('.styled-submit.fll', 'click', function(){
					
					var quantity = $('.quantity').val();
					$('.ff input[name=form_hidden_10]').val(quantity);
					$('.ff input[name=form_hidden_11]').val('<?=$arResult["PROPERTY_67"];?>');
					//return false;
				});

				$('.ff').delegate('.hide_ff', 'click', function(){
					$('.ff').hide();
					$('#show_ff').css('opacity', '1');
					//return false;
				});
...
<style>
	.bx_item_detail_size.full ul {
		width: 100% !important;
		margin-left: 0 !important;
	}

	.bx_item_detail_size.full ul li {
		width: 20% !important;
	}
	.pr_sl {
...
<div class="clearfix" id="product-content" itemscope itemtype="http://schema.org/Product">
	<div class="bx_item_detail <?=$templateData['TEMPLATE_CLASS'];?>" id="<?=$arItemIDs['ID'];?>">

		<meta itemprop="url" content="http://<?=SITE_SERVER_NAME.$arResult["DETAIL_PAGE_URL"];?>" />
		<meta itemprop="image" content="http://<?=SITE_SERVER_NAME.$arResult["DETAIL_PICTURE"]["SRC"];?>" />
	
		<ul id="product-details">

			<li id="product-title">
				<h1 itemprop="name"><?=(isset($arResult['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE']) && $arResult['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE'] != '' ? $arResult['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE'] : $arResult['NAME']);?></h1>
				<? if (!empty($arResult['DISPLAY_PROPERTIES']['VENDOR_ID']['DISPLAY_VALUE'])) { ?>
				<h2 itemprop="brand"><?=$arResult['DISPLAY_PROPERTIES']['VENDOR_ID']['DISPLAY_VALUE'];?></h2>
				<? } ?>
			</li>
			...
			<? if (isset($arResult['MIN_PRICE']) && !empty($arResult['MIN_PRICE'])) { ?>
				<li id="product-prices" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
					...
					<link itemprop="availability" href="http://schema.org/<?=($canBuy) ? 'InStock' : 'OutOfStock';?>" />
		
					<p id="product-price" class="smooth">
						<? $boolDiscountShow = (0 < $arResult['MIN_PRICE']['DISCOUNT_DIFF']); ?>
						<span class="product-price" id="<?=$arItemIDs['OLD_PRICE'];?>" style="display: <?=($boolDiscountShow ? '' : 'none'); ?>"><?=($boolDiscountShow ? $arResult['MIN_PRICE']['PRINT_VALUE'] : '');?><br></span>
						<span class="product-price" itemprop="price" id="<?=$arItemIDs['PRICE'];?>"><?=$arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE'];?></span>
						<span class="product-price" id="<?=$arItemIDs['DISCOUNT_PRICE'];?>" style="display: <?=($boolDiscountShow ? '' : 'none'); ?>"><br><?=($boolDiscountShow ? GetMessage('CT_BCE_CATALOG_ECONOMY_INFO',array('#ECONOMY#'=>$arResult['MIN_PRICE']['PRINT_DISCOUNT_DIFF'])) : ''); ?></span>
					</p>
				</li>
			<? } ?>
			...
			<? if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) { ?>
				...
				<li id="product-variants" class="clearfix">
					<div class="item_info_section" style="padding: 0 15px 0 0;" id="<?=$arItemIDs['PROP_DIV'];?>">
					<?
						...
						if ('TEXT' == $arProp['SHOW_MODE'])
						{
							...
							<div class="<?=$strClass;?>" id="<?=$arItemIDs['PROP'].$arProp['ID']; ?>_cont">
								...
								<div class="bx_size_scroller_container">
									<div class="bx_size">
										<ul id="<?=$arItemIDs['PROP'].$arProp['ID']; ?>_list" style="width: <?=$strWidth; ?>; margin-left: 0%;">
										<?
										foreach ($arProp['VALUES'] as $arOneValue)
										{
											$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
											?>
											<li data-treevalue="<?=$arProp['ID'].'_'.$arOneValue['ID']; ?>" data-onevalue="<?=$arOneValue['ID'];?>" style="width: <?=$strOneWidth;?>; display: none; border: 0;">
											<i title="<?=$arOneValue['NAME']; ?>"></i><span class="cnt" title="<?=$arOneValue['NAME']; ?>"><?=$arOneValue['NAME']; ?></span></li>
											<?
										}
										?>
										</ul>
									</div>
									<div class="bx_slide_left pr_sl" style="display:<?=$strSlideStyle; ?>" id="<?=$arItemIDs['PROP'].$arProp['ID']; ?>_left" data-treevalue="<?=$arProp['ID']; ?>"></div>
									<div class="bx_slide_right pr_sl" style="display:<?=$strSlideStyle; ?>" id="<?=$arItemIDs['PROP'].$arProp['ID']; ?>_right" data-treevalue="<?=$arProp['ID']; ?>"></div>
								</div>
							</div>
							<?
						}
						else if ('PICT' == $arProp['SHOW_MODE'])
						{
								...
								<div class="bx_scu_scroller_container">
									<div class="bx_scu">
										...
										foreach ($arProp['VALUES'] as $arOneValue)
										{
											$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
											?>
											<li data-treevalue="<?=$arProp['ID'].'_'.$arOneValue['ID'] ?>" data-onevalue="<?=$arOneValue['ID']; ?>" style="width: <?=$strOneWidth;?>; padding-top: <?=$strOneWidth;?>; display: none;  border: 0;">
									...
									<div class="bx_slide_left" style="<?=$strSlideStyle; ?>" id="<?=$arItemIDs['PROP'].$arProp['ID']; ?>_left" data-treevalue="<?=$arProp['ID'];?>"></div>
									<div class="bx_slide_right" style="<?=$strSlideStyle; ?>" id="<?=$arItemIDs['PROP'].$arProp['ID']; ?>_right" data-treevalue="<?=$arProp['ID'];?>"></div>
								</div>
							</div>
							<?
						}
				...
				<? } else { ?>
				<input type="hidden" id="<?=$arItemIDs['PROP_DIV'];?>">
				<? } ?>
			...
			<li id="product-add1">
				<div class="product-padding">
					<span id="<?=$arItemIDs['BASKET_ACTIONS'];?>" class="<?=($pr ? 'offpror' : ''); ?>" style="display: <?=($arResult["PROPERTY_66"]!=39 ? '' : 'none'); ?>;">
						<label for="quantity">Количество</label>
						<input id="<?=$arItemIDs['QUANTITY']; ?>" type="number" name="quantity" class="styled-input" value="1" />
						<br /><br />
						<input type="button" name="add" id="<?=$arItemIDs["BUY_LINK"];?>" value="Купить" class="btn styled-submit">
					</span>
					
					<span id="<?=$arItemIDs['NOT_AVAILABLE_MESS'];?>" class="<?=($pr ? 'onpror' : ''); ?>" style="display: <?=($arResult["PROPERTY_66"]==39 ? '' : 'none'); ?>;">
						<label for="quantity">Количество</label>
						<input id="<?=$arItemIDs['QUANTITY']; ?>" class="quantity" type="number" name="quantity" class="styled-input" value="1" />
						<br /><br />
						<input type="button" id="show_ff" name="add" value="Предзаказ" class="btn styled-submit">
						<p class="preorder-form-descr">Ожидаемая дата поставки - <?=$arResult["PROPERTY_67"];?></p>
						<span id="<?=$arItemIDs['SUBSCRIPTION'];?>" class="ff" style="display: <?=(!$canSubscription ? 'none' : ''); ?>;">
						</span>
		...
		<? if (sizeof($arResult["LINKED_ELEMENTS"]) > 0) { ?>
		<div id="product-photos">
			<div id="product-photo-container">
				<? foreach($arResult["LINKED_ELEMENTS"] as $arPhotos) { ?>
				<?
				$PHOTOS_NAME = trim($arPhotos["~NAME"]);
				if (empty($PHOTOS_NAME)) {$PHOTOS_NAME = $arResult["~NAME"];}
				$DETAIL_PICTURE_RESIZE = CFile::ResizeImageGet($arPhotos["DETAIL_PICTURE"],array("width"=>500,"height"=>1000),BX_RESIZE_IMAGE_PROPORTIONAL);
				?>
				<a class="gallery" href="<?=$arPhotos["DETAIL_PICTURE"]["SRC"];?>" rel="gallery"><img src="<?=$DETAIL_PICTURE_RESIZE["src"];?>" alt="<?=$PHOTOS_NAME;?>" /></a>
				<? } ?>
			...
			<ul id="product-photo-thumbs" class="clearfix">
				...
				if (empty($PHOTOS_NAME)) {$PHOTOS_NAME = $arResult["~NAME"];}
				?>
				<li class="product-photo-thumb"><img src="<?=$arPhotos["PREVIEW_PICTURE"]["SRC"];?>" alt="<?=$PHOTOS_NAME;?>" /> </li>
				<? } ?>
			</ul>
		</div><!-- #product-photos -->
		<? } ?>
		...
		if ($arResult['SHOW_SLIDER'])
		{
			if (!isset($arResult['OFFERS']) || empty($arResult['OFFERS']))
			{
				...
				?>
				<div class="<?=$strClass; ?>" id="<?=$arItemIDs['SLIDER_CONT_ID']; ?>">
					<div class="bx_slider_scroller_container">
						<div class="bx_slide">
							<ul style="width: <?=$strWidth; ?>;" id="<?=$arItemIDs['SLIDER_LIST']; ?>"></ul>
						</div>
			...
			else
			{
				foreach ($arResult['OFFERS'] as $key => $arOneOffer)
				{
					...
					?>
					<div class="<?=$strClass; ?>" id="<?=$arItemIDs['SLIDER_CONT_OF_ID'].$arOneOffer['ID']; ?>">
						<div class="bx_slider_scroller_container">
							<div class="bx_slide">
								<ul style="width: <?=$strWidth; ?>;" id="<?=$arItemIDs['SLIDER_LIST_OF_ID'].$arOneOffer['ID']; ?>"></ul>
							</div>
							<div class="bx_slide_left" id="<?=$arItemIDs['SLIDER_LEFT_OF_ID'].$arOneOffer['ID'] ?>" style="<?=$strSlideStyle; ?>" data-value="<?=$arOneOffer['ID']; ?>"></div>
							<div class="bx_slide_right" id="<?=$arItemIDs['SLIDER_RIGHT_OF_ID'].$arOneOffer['ID'] ?>" style="<?=$strSlideStyle; ?>" data-value="<?=$arOneOffer['ID']; ?>"></div>
						</div>
					</div>
					<?
				}
