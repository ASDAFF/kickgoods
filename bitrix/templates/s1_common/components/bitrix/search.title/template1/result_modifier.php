...
foreach($arResult["SEARCH"] as $i=>$arItem)
{
		...
		case "iblock":
			...
			elseif(CModule::IncludeModule('iblock'))
			{
				...
				if(substr($arItem["ITEM_ID"], 0, 1) !== "S")
				{
					...
					{
						$PREVIEW_PICTURE = $arElement["PROPERTY_CML2_LINK_PREVIEW_PICTURE"];
					}
	...
	{
		$PREVIEW_PICTURE_RESIZE = CFile::ResizeImageGet($PREVIEW_PICTURE,array("width"=>50,"height"=>50),BX_RESIZE_IMAGE_PROPORTIONAL);
		$arResult["SEARCH"][$i]["PREVIEW_PICTURE"] = $PREVIEW_PICTURE_RESIZE["src"];
	}
}
