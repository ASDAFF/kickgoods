function BasketServicesAddRemove(BASKET_ID,PRODUCT_ID,PRODUCT_URL)
{
	if ($('#BASKET_SERVICES_'+PRODUCT_ID).prop('checked'))
	{
		location.href=PRODUCT_URL+'?action=BUY&id='+PRODUCT_ID+'&quantity=1';
	}
	else
	{
		location.href='/cart/?action=delete&id='+BASKET_ID;
	}
}
...
function checkOut()
{
	var ORDER_FIELDS_GREETING_TEXT = $('#ORDER_FIELDS_GREETING_TEXT').val();
	if (ORDER_FIELDS_GREETING_TEXT === undefined)
	{
		ORDER_FIELDS_GREETING_TEXT = '';
	}
	
	BX.ajax
	(
		{
			url: basketJSParams.TEMPLATE_FOLDER+'/ajax.php',
			method: 'POST',
			data:
			{
				'act':'ORDER_FIELDS',
				'ORDER_FIELDS_GREETING_TEXT':''+ORDER_FIELDS_GREETING_TEXT+''
			}
		}
	);
