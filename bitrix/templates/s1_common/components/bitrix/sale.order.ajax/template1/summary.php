...
<div class="bx_ordercart">
					...
					?>
						<td class="itemphoto">
							<div class="bx_ordercart_photo_container">
								...
								{
									$PICTURE = CFile::GetPath($arData["data"]["PREVIEW_PICTURE"]);
								}

								if (strlen($arData["data"]["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arData["data"]["DETAIL_PAGE_URL"] ?>"><?endif;?>
									<div class="bx_ordercart_photo" style="background-image:url('<?=$PICTURE;?>')"></div>
	...
	<? if (sizeof($BASKET_SERVICES) > 0) { ?>
		<br>
		<h4>Дополнительные услуги</h4>
		<div class="bx_ordercart_order_service_table_container">
			<table>
				<tbody>
					<? foreach($BASKET_SERVICES as $arItem) { ?>
					<tr>
						<td><?=$arItem["NAME"];?><?=($arItem["QUANTITY"] > 1 ? ' x '.$arItem["QUANTITY"] : '');?>:</td>
						<td><?=$arItem["SUM"];?></td>
					</tr>
					<? } ?>
				</tbody>
	...
	<div class="bx_ordercart_order_pay">
		<div class="bx_ordercart_order_pay_right">
			<table class="bx_ordercart_order_sum">
				<tbody>
					<tr>
						<td class="custom_t1 itog" colspan="<?=$colspan?>"><?=GetMessage("SOA_TEMPL_SUM_WEIGHT_SUM")?></td>
						<td class="custom_t2 price"><?=$arResult["ORDER_WEIGHT_FORMATED"]?></td>
					</tr>
					<tr>
						<td class="custom_t1 itog" colspan="<?=$colspan?>"><?=GetMessage("SOA_TEMPL_SUM_SUMMARY")?></td>
					...
					if (doubleval($arResult["DISCOUNT_PRICE"]) > 0)
					{
						?>
						<tr>
							<td class="custom_t1 itog" colspan="<?=$colspan?>"><?=GetMessage("SOA_TEMPL_SUM_DISCOUNT")?><?if (strLen($arResult["DISCOUNT_PERCENT_FORMATED"])>0):?> (<?echo $arResult["DISCOUNT_PERCENT_FORMATED"];?>)<?endif;?>:</td>
					...
					if(!empty($arResult["TAX_LIST"]))
					{
						foreach($arResult["TAX_LIST"] as $val)
						{
							?>
							<tr>
								<td class="custom_t1 itog" colspan="<?=$colspan?>"><?=$val["NAME"]?> <?=$val["VALUE_FORMATED"]?>:</td>
					...
					if (doubleval($arResult["DELIVERY_PRICE"]) > 0)
					{
						?>
						<tr>
							<td class="custom_t1 itog" colspan="<?=$colspan?>"><?=GetMessage("SOA_TEMPL_SUM_DELIVERY")?></td>
					...
					if (strlen($arResult["PAYED_FROM_ACCOUNT_FORMATED"]) > 0)
					{
						?>
						<tr>
							<td class="custom_t1 itog" colspan="<?=$colspan?>"><?=GetMessage("SOA_TEMPL_SUM_PAYED")?></td>
					...
					?>
						<tr>
							<td class="custom_t1 fwb itog" colspan="<?=$colspan?>"><?=GetMessage("SOA_TEMPL_SUM_IT")?></td>
