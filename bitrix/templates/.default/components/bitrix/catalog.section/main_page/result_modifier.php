...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		### Получение минимальной цены в независимости от наличия товара

		if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			$arOffersTmp = array();
			foreach($arItem['OFFERS'] as $arItemOffersKey=>$arItemOffersVal)
			{
				$arItemOffersVal["CAN_BUY"] = "Y";

				$arOffersTmp[$arItemOffersKey] = $arItemOffersVal;
			}
			
			$arItem['MIN_PRICE'] = CIBlockPriceTools::getMinPriceFromOffers($arOffersTmp,$boolConvert ? $arResult['CONVERT_CURRENCY']['CURRENCY_ID'] : $strBaseCurrency);
		}
...
{
	$result_vendors = CIBlockElement::GetList(array("SORT"=>"ASC"),array("IBLOCK_ID"=>4,"CODE"=>$_REQUEST["VENDORS_CODE"]),false,array("nTopCount"=>1),array("NAME"));
	if ($result_vendors->SelectedRowsCount() > 0)
	{
		while($arVendors = $result_vendors->GetNext())
		{
			$arResult["VENDORS_NAME"] = $arVendors["NAME"];
		}
	}
}
...
$this->__component->SetResultCacheKeys(array('VENDORS_NAME'));
